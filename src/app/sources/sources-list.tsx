    ...
    <div className="space-y-6">
      ...
      <ul className="space-y-2">
        {sources.map((s) => (
          <li
            key={s.id}
            className="bg-white border border-zinc-200 rounded-xl p-4 flex items-center justify-between shadow-sm"
          >
            <div>
              <span className="font-medium text-zinc-800 capitalize">{s.type}</span>
              {s.type === "rss" && (
                <span className="text-sm text-zinc-500 ml-2 truncate max-w-md inline-block align-bottom">
                  {(s.config as { url?: string }).url}
                </span>
              )}
              {s.type === "arxiv" && (
                <span className="text-sm text-zinc-500 ml-2">
                  {(s.config as { category?: string }).category}
                  {(s.config as { keyword?: string }).keyword &&
                    ` + ${(s.config as { keyword?: string }).keyword}`}
                </span>
              )}
            </div>
